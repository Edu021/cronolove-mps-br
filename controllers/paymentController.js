...
const paymentController = {
            ...
            if (req.files && req.files.length > 0) {
                req.files.forEach((file) => {
                    ...
                    const query = `
                        ...
                        (session_id, image_name, coupleName, music, message, started_at, plan) 
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                    `;
    ...
    paymentNotify(req, res) {
        ...
        // Verifique o tipo de evento (você pode escolher o que deseja escutar, como "payment")
        if (data.type === 'payment') {
            const paymentId = data.data.id;
    
            mercadopago.payment.get(paymentId).then(response => {
                const paymentInfo = response.response;
                const status = paymentInfo.status;
    
                if (status === 'approved') {
                    const { payer_email, status, transaction_details, id } = paymentInfo;
    
                    // Aqui você pode acessar outras informações do pagamento, como o email do pagador, etc.
                    console.log(`Pagamento aprovado: ${id} - ${payer_email}`);
    
                    // Agora, registre a página da pessoa no banco de dados
                    const { coupleName, message, music, started_at, plan } = data.data.metadata;  // Dados que você passou ao criar a preferência
                    ...
                    PageModel.createPage(coupleName, music, message, started_at, plan, (err, result) => {
                        ...
                        emailController.sendEmail(`${process.env.BASE_URL}/${result.insertId}/${sanitazedName}`, payer_email);
